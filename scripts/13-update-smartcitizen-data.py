...
def main():
    ...
    for d in df_pip.loc[df_pip.type == 'Device']['id']:
        ...
        for s in df_device.id.unique():
            ...
            df_mes['device_id'] = d
            df_mes['param'] = df_device.loc[df_device.id ==
                                            s]['description'].values[0]
    ...
    gdf_db = aqiGDL.gdf_from_db('smartcitizen', 'public')

    # append new download and delete duplicates
    gdf_sck = gdf_sck.append(gdf_db)
    gdf_sck.drop_duplicates(inplace=True)
