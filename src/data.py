...
def simaj_download(year_start=2014, year_end=2019):
    ...
    for year in range(year_end-year_start+1):
        ...
        save_path = os.path.join('../gdl-aire/gdlaire/data/raw/', 'datos_'+str(year_start+year)+'.xlsx')

        urllib.request.urlretrieve('http://siga.jalisco.gob.mx/aire/reportes/datos_'+str(year_start+year)+'.xlsx',save_path)
...
def database_clean (interval='hour'):
    ...
    for file in os.listdir(dir_gdl):
        ...
        date_array=np.array(daterange(start_date, end_date, interval=interval)) #creates array for every hour of a given year

        #creates new df in which air quality data for all stations will be saved
        df = pd.DataFrame(index=np.array(range(0,len(date_array))), columns=['FECHA','HORA','O3','PM10','CO','NO2','SO2'])
        ...
        for s in sheets:
            ...
            gdl_data = pd.read_excel(dir_gdl+file, sheet_name = s).replace(r'^\s*$',np.nan,regex=True)

            gdl_data.rename(columns={gdl_data.columns[0]:'FECHA',gdl_data.columns[1]:'HORA'}, inplace=True)

            ###fixes column names
            if int(year)==2014 and  s == 'Las Pintas':
                gdl_data.rename(columns={gdl_data.columns[6]:'SO2'}, inplace=True)
            
            if int(year) == 2018 and s != 'Vallarta':
                gdl_data.rename(columns={gdl_data.columns[2]:'O3'}, inplace=True)

            ###fixes date
            if int(year) == 2018 and s == 'Centro':
                gdl_data['FECHA'].iloc[4112] = datetime.strptime('2018-06-21', '%Y-%m-%d')


            gdl_data.columns = gdl_data.columns.str.replace(' ', '') #removes spaces from columns

            gdl_data.columns = [col.replace(':', '') for col in gdl_data.columns] #removes : from columns

            gdl_data = gdl_data[['FECHA','HORA','O3','NO2','SO2','PM10','CO']] #filters data
            ...
            for i in range (len(gdl_data)):
                        ...
                        try:
                            ...
                            gdl_data['HORA'].iloc[i] = '{:02d}:{:02d}'.format(gdl_data['HORA'].iloc[i].hour, gdl_data['HORA'].iloc[i].minute)
                            ...
                            try:
                                ...
                                gdl_data['HORA'].iloc[i] = '{:02d}:{:02d}'.format(time_datetime.hour, time_datetime.minute)
                                ...
                                date_datetime = datetime.strptime(prev_datetime, '%Y-%m-%d %H:%M')

                                new_datetime = date_datetime + timedelta(hours=1)

                                gdl_data['FECHA'].iloc[i] = new_datetime.date()
                                gdl_data['HORA'].iloc[i] = '{:02d}:{:02d}'.format(new_datetime.hour, new_datetime.minute)


            #stacks gdl DataFrame so for every date there are 5 rows with criterion pollutants
            gdl_stack = pd.DataFrame(gdl_data.set_index(['FECHA','HORA']).stack([0], dropna=False))
            ...
            if interval == 'day':
                ...
                gdl_stack = gdl_stack.groupby(['FECHA','HORA','PARAM']).mean()

            #adds data from gdl_stack for a specified year to all_data which 
            #will contain information for every year
            df = pd.merge(df, gdl_stack, how='left',left_index=True, right_index=True)
